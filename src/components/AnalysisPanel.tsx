        ...
        {factors.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xl text-green-400 font-semibold">
              {translations.noIssuesTitle}
            </p>
            <p className="text-gray-400 mt-2">{translations.noIssuesMessage}</p>
          </div>
        ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-2">
                      <span className="text-xs text-gray-400">
                        {translations.countLabel}: <span className="font-mono text-white">{factor.count}</span>
                      </span>
                      <span className="text-xs text-gray-400">
                        {translations.severityLabel}: <span className="font-mono text-orange-400">{factor.severity}</span>
                      </span>
                    </div>
                  </div>
                  <div
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-2xl font-bold">{translations.suggestionTitle}</h3>
          </div>
          <button
            onClick={() => setShowRefactor(!showRefactor)}
            className="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-sm font-semibold transition-colors"
          >
            {showRefactor ? translations.hideButton : translations.showButton}
          </button>
        </div>
